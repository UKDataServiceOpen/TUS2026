...
We will open the Stata dataset using `read_dta()` from the `haven` package, which retains both the original numeric values and  Stata's variable and value labels.
...
```{r clean}
ep<-(
    read_dta("~/Data/mtus/mtus_teach_ep.dta") %>%
...
```
 ...
 We can visualise  the first few lines of raw data, without the value labels.
 More information about the variable definition is available in the MTUS documentation
...
<!-- class(ep$main) -->
<!-- ```  -->
We can then explore a few variables, for example the number of episodes by country
...
```{r loadagg}
d<-read_dta("~/Data/mtus/mtus_teach_ind.dta")
