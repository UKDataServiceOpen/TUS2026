...
- Each line records the time unit of the diary 
- Largest datasets
- 144 observations (ie 10 minute slots) per day
- Requires a slot identifier variables
...
- However, episode or time slots  datasets are also available in wide format:
...
- Each line represents a day, each time slot  and its attributes (activities, location, copresence are recorded in one variable)
- Lots of variable, less intuitive but more parsimonious than the long format



## Common time diary variables
	
- Most time diary datasets include similar variables:
...
- In many time use surveys, respondents still fill in their time diary by hand
